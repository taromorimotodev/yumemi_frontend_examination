...
import { PrefecturePopulationProvider, usePrefecturePopulationContext } from './PrefecturePopulationContext';

const TestComponent: React.FC = () => {
  const { selectedPrefectures, setSelectedPrefectures, selectedPopulation, setSelectedPopulation } = usePrefecturePopulationContext();
...
describe('PrefecturePopulationContext', () => {
  ...
  it('updates values correctly', () => {
    ...
    expect(screen.getByTestId('selectedPopulation').textContent).toBe('1000000');
  });

  it('throws error when used outside of provider', () => {
    const consoleError = jest.spyOn(console, 'error').mockImplementation(() => {});

    expect(() => render(<TestComponent />)).toThrow('usePrefecturePopulationContext must be used within a PrefectureProvider');
